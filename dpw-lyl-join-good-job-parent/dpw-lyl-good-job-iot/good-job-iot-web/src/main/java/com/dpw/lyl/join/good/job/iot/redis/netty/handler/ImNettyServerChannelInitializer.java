...
@Slf4j
@Component
public class ImNettyServerChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    @Resource
    private ImTravelPermissionWebSocketHandler imTravelPermissionWebSocketHandler;

    @Resource
    private ImNettyServerHandler imNettyServerHandler;
    ...
    protected void initChannel(SocketChannel ch) throws Exception {
        ...
        //鉴权登录信息
        ch.pipeline().addLast(imTravelPermissionWebSocketHandler);
        ch.pipeline().addLast(new WebSocketServerProtocolHandler(ImNettyEventType.WEBSOCKET_PATH));
        ch.pipeline().addLast(imNettyServerHandler);
